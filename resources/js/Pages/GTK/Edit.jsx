...
export default function Edit({ auth, gtk }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        nik: gtk.nik || '',
        no_kk: gtk.no_kk || '',
        jenis_kelamin: gtk.jenis_kelamin || 'L',
        tempat_lahir: gtk.tempat_lahir || '',
        tanggal_lahir: gtk.tanggal_lahir ? gtk.tanggal_lahir.split('T')[0] : '',
        nama_ibu_kandung: gtk.nama_ibu_kandung || '',
        agama: gtk.agama || '',
        kewarganegaraan: gtk.kewarganegaraan || 'WNI',
        status_perkawinan: gtk.status_perkawinan || 'Belum Kawin',
        nama_suami_istri: gtk.nama_suami_istri || '',
        pekerjaan_suami_istri: gtk.pekerjaan_suami_istri || '',

        // Alamat & Kontak
        alamat: gtk.alamat || '',
        rt: gtk.rt || '',
        rw: gtk.rw || '',
        nama_dusun: gtk.nama_dusun || '',
        desa_kelurahan: gtk.desa_kelurahan || '',
        kecamatan: gtk.kecamatan || '',
        kode_pos: gtk.kode_pos || '',
        no_hp: gtk.no_hp || '',
        email: gtk.email || '',
        lintang: gtk.lintang || '',
        bujur: gtk.bujur || '',
        ...
        nip: gtk.nip || '',
        nuptk: gtk.nuptk || '',
        jenis_ptk: gtk.jenis_ptk || 'Guru Mapel',
        status_kepegawaian: gtk.status_kepegawaian || 'GTY',
        sk_pengangkatan: gtk.sk_pengangkatan || '',
        tmt_pengangkatan: gtk.tmt_pengangkatan ? gtk.tmt_pengangkatan.split('T')[0] : '',
        lembaga_pengangkatan: gtk.lembaga_pengangkatan || '',
        sk_cpns: gtk.sk_cpns || '',
        tmt_cpns: gtk.tmt_cpns ? gtk.tmt_cpns.split('T')[0] : '',
        pangkat_golongan: gtk.pangkat_golongan || '',
        sumber_gaji: gtk.sumber_gaji || '',

        // Lainnya
        tugas_tambahan: gtk.tugas_tambahan || '',
        npwp: gtk.npwp || '',
        nama_wajib_pajak: gtk.nama_wajib_pajak || '',
        bank: gtk.bank || '',
        nomor_rekening_bank: gtk.nomor_rekening_bank || '',
        rekening_atas_nama: gtk.rekening_atas_nama || '',
        sudah_lisensi_kepala_sekolah: gtk.sudah_lisensi_kepala_sekolah || 'Tidak',
        keahlian_braille: gtk.keahlian_braille || 'Tidak',
        keahlian_bahasa_isyarat: gtk.keahlian_bahasa_isyarat || 'Tidak',
        foto: null,
    });
            ...
            header={
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="font-bold text-2xl text-gray-800">Edit Data GTK</h2>
                            <p className="text-gray-500 text-sm">Perbarui data Guru dan Tenaga Kependidikan</p>
                        </div>
                    </div>
                </div>
            }
                        ...
                        {activeTab === 'lainnya' && (
                            <div className="grid grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div className="col-span-2">
                                    ...
                                    {gtk.foto && (
                                        <div className="mb-4">
                                            <p className="text-xs text-gray-400 mb-2">Foto Saat Ini:</p>
                                            <img src={`/storage/${gtk.foto}`} alt="Current" className="h-20 w-20 rounded-full object-cover border-2 border-white shadow-sm" />
                                        </div>
                                    )}
                    ...
                    <div className="bg-gray-50 px-8 py-6 border-t border-gray-100 flex justify-between items-center">
                        ...
                        <div className="flex gap-3">
                            {activeTab !== tabs[tabs.length - 1].id ? (
                                ...
                                >
                                    {processing ? 'Menyimpan...' : <><Save size={18} /> Perbarui Data GTK</>}
                                </button>
                            )}
                        </div>
                    </div>
